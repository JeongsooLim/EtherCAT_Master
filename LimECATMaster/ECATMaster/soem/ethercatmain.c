...
uint16 ecx_statecheck(ecx_contextt *context, uint16 slave, uint16 reqstate, int timeout)
{
    ...
    context->slavelist[slave].state = rval;
    printf("[Lim] ALStatus(%d): %d ", slave, rval);
...
int ecx_send_processdata_group(ecx_contextt *context, uint8 group)
{
    ...
    if (length){
        ...
        if(context->grouplist[group].blockLRW){
            ...
            if(context->grouplist[group].Ibytes){
                ...
                do{
                    ...
                    w1 = LO_WORD(LogAdr);
                    w2 = HI_WORD(LogAdr);
                    printf("[Lim] CurrentSeg: %d  LogStartAddr: %d   ", currentsegment, LogAdr);
        ...
        {
            //printf("LRW Not Blocked  OBytes: %d, IBytes: %d", context->grouplist[group].Obytes, context->grouplist[group].Ibytes);
            if (context->grouplist[group].Obytes){
                data = context->grouplist[group].outputs;
            }else{
                //printf("Ibytes  ");
            ...
            do{
                ...
                w1 = LO_WORD(LogAdr);
                w2 = HI_WORD(LogAdr);
                //printf("[Lim] Seg: %d, SubLength: %d, LO: %d, HI: %d   ", currentsegment, sublength, w1, w2);
                //printf("[Lim] CurrentSeg: %d  LogStartAddr: %d   ", currentsegment, LogAdr);
